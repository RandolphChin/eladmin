            ...
            <#if column.queryType != 'BetWeen'>
              <#if column.formType = 'Select'>
                <#if (column.dictName)?? && (column.dictName)!="">
                  <el-select v-model="query.${column.changeColumnName}" clearable filterable
  ...
  <#if betweens??>
    ...
    <#if columns??>
      <#list columns as column>
        <#if column.formShow>
          <el-form-item label="<#if column.remark != ''>${column.remark}<#else>${column.changeColumnName}</#if>"<#if column.istNotNull> prop="${column.changeColumnName}"</#if>>
            ...
            <#else>
            <el-date-picker v-model="form.${column.changeColumnName}" type="datetime" style="width: 370px;" align="center"/>
            </#if>
          </el-form-item>
        </#if>
      </#list>
      ...
      <el-table ref="table" v-loading="crud.loading" :data="crud.data" size="small" style="width: 100%;" @selection-change="crud.selectionChangeHandler">
        ...
        <el-table-column v-if="checkPer([defaultAdminUser,'${changeClassName}:edit','${changeClassName}:del'])" label="操作" width="150px" align="center">
          <template v-slot="scope">
            <udOperation
              :data="scope.row"
...
export default {
  ...
  data() {
    return {
      permission: {
        add: [defaultAdminUser, '${changeClassName}:add'],
        edit: [defaultAdminUser, '${changeClassName}:edit'],
        del: [defaultAdminUser, '${changeClassName}:del']
      },
      rules: {
        <#if isNotNullColumns??>
