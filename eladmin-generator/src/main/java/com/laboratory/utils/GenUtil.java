...
import com.laboratory.domain.GenConfig;
...
import static com.laboratory.utils.FileUtil.SYS_TEM_DIR;
...
public class GenUtil {
    ...
    public static String download(List<ColumnInfo> columns, GenConfig genConfig) throws IOException {
        ...
        for (String templateName : templates) {
            Template template = engine.getTemplate("generator/backend/" + templateName + ".ftl");
            String filePath = getAdminFilePath(templateName, genConfig, genMap.get("className").toString(), tempPath + "lims8000" + File.separator);
    ...
    public static void generatorCodeForEladmin(List<ColumnInfo> columnInfos, GenConfig genConfig) throws IOException {
        Map<String,Object> genMap = getGenMap(columnInfos, genConfig);
        TemplateEngine engine = TemplateUtil.createEngine(new TemplateConfig("template", TemplateConfig.ResourceMode.CLASSPATH));
        // 生成后端代码
        List<String> templates = getAdminTemplateNamesForEladmin();
        for (String templateName : templates) {
            Template template = engine.getTemplate("generator/backend/"+templateName+".ftl");
            String filePath = getAdminFilePathForEladmin(templateName,genConfig,genMap.get("className").toString(),System.getProperty("user.dir"));
